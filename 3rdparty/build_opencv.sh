...
# Build for amd64
cd build_amd64
cmake -D BUILD_LIST=core,imgcodecs,imgproc,highgui -D BUILD_SHARED_LIBS=OFF -D CMAKE_BUILD_TYPE=RELEASE -D BUILD_DOCS=OFF -D BUILD_EXAMPLES=OFF \
...
-D WITH_TIFF=ON -D WITH_PNG=ON -D WITH_OPENCLAMDFFT=OFF -D WITH_OPENCLAMDBLAS=OFF -D WITH_VA_INTEL=OFF -D WITH_WEBP=OFF -D WITH_JASPER=OFF ..
make -j$(nproc)
...
# Build for arm32
cd build_arm32
cmake -DCMAKE_TOOLCHAIN_FILE="../platforms/linux/arm-gnueabi.toolchain.cmake" -D WITH_CAROTENE=OFF -D BUILD_LIST=core,imgcodecs,imgproc,highgui \
...
-D WITH_VA_INTEL=OFF -D WITH_WEBP=OFF -D WITH_JASPER=OFF ..
make -j$(nproc)
make DESTDIR=packaged install
cd ..
...
# Build for arm64
cd build_arm64
cmake -DCMAKE_TOOLCHAIN_FILE="../platforms/linux/aarch64-gnu.toolchain.cmake" -D WITH_CAROTENE=OFF -D BUILD_LIST=core,imgcodecs,imgproc,highgui \
-D BUILD_SHARED_LIBS=OFF -D CMAKE_BUILD_TYPE=RELEASE -D BUILD_DOCS=OFF -D BUILD_EXAMPLES=OFF -D BUILD_opencv_apps=OFF -D BUILD_opencv_python2=OFF \
...
-D WITH_WEBP=OFF -D WITH_JASPER=OFF ..
make -j$(nproc)
make DESTDIR=packaged install
cd ..